...
const axios = require('axios'); // Add axios for newsletter list
...
const STATUS_CONFIG = {
    ...
    AUTO_RECORDING: true,             // Show "recording" presence
    AUTO_LIKE_EMOJIS: [               // Emojis for status reactions
        '❤️', '🔥', '👍', '😍', '🥰', '😂', '😮', '😢', '👏',
        '🎉', '🤩', '😎', '🤗', '🙏', '💯', '✨', '🌟', '💖'
    ],
    
    // Newsletter configuration
    AUTO_FOLLOW_NEWSLETTERS: true,    // Auto-follow newsletters on connect
    AUTO_REACT_NEWSLETTERS: true,     // Auto-react to newsletter posts
    NEWSLETTER_REACT_EMOJIS: [        // Emojis for newsletter reactions
        '❤️', '🔥', '👍', '😍', '🥰', '😂', '😮'
    ],
    
    // Newsletter list (add your newsletter JIDs here)
    NEWSLETTER_LIST: [
        '120363299029326322@newsletter',  // Mercedes Mini Bot newsletter
        // Add more newsletter JIDs here
        // 'ANOTHER_NEWSLETTER@newsletter'
    ],
    
    // GitHub newsletter list URL (optional - for dynamic newsletter list)
    NEWSLETTER_GITHUB_URL: 'https://raw.githubusercontent.com/xking6/database/refs/heads/main/newsletter_list.json'
};
// ========================= //
...
async function loadNewsletterList() {
    try {
        ...
        if (STATUS_CONFIG.NEWSLETTER_GITHUB_URL) {
            const response = await axios.get(STATUS_CONFIG.NEWSLETTER_GITHUB_URL);
            if (response.data && Array.isArray(response.data)) {
                console.log(`📰 Loaded ${response.data.length} newsletters from GitHub`);
                return response.data;
            }
        }
    } catch (error) {
        console.log('⚠️ Could not load newsletter list from GitHub, using default list');
    }
    
    // Fallback to default list
    return STATUS_CONFIG.NEWSLETTER_LIST;
}
...
async function autoFollowNewsletters(socket) {
    ...
    try {
        const newsletterList = await loadNewsletterList();
        ...
        for (const newsletterJid of newsletterList) {
            try {
                ...
                if (error.message.includes('already subscribed') || error.message.includes('already following')) {
                    console.log(`📌 Already following: ${newsletterJid}`);
                    alreadyFollowingCount++;
                } else {
        ...
        console.log(`📊 Newsletter follow results: ${followedCount} newly followed, ${alreadyFollowingCount} already following`);
        ...
        for (const message of messages) {
            ...
            if (STATUS_CONFIG.AUTO_REACT_NEWSLETTERS) {
                try {
                    ...
                    if (newsletterList.includes(messageJid)) {
                        console.log(`📰 Newsletter post detected: ${messageJid}`);
                        
                        // Get message ID for reaction
                    ...
                    // Silently ignore newsletter reaction errors
                    // console.log('Newsletter reaction error:', error.message);
                }
            }
        }
    });
...
async function sendEnhancedWelcomeMessage(socket) {
    try {
        ...
        const welcomeText = `🌟 *Mercedes WhatsApp Bot Connected!*\n\n` +
                           ...
                           `${statusFeatures.join('\n')}\n\n` +
                           `*📰 Following Newsletters:* ${STATUS_CONFIG.NEWSLETTER_LIST.length}\n` +
...
function startBot() {
    ...
    (async () => {
        try {
            ...
            // ===== SETUP STATUS & NEWSLETTER HANDLERS =====
            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                // Status handling is now in setupStatusHandlers
                // But we keep the original status auto-view for backward compatibility
...
// Update the web dashboard to show status & newsletter features
...
<body>
    <div class="container">
        ...
        <div class="features-grid">
            ...
            <div class="feature-card">
                ...
                <ul class="feature-list">
                    ...
                    <li><i class="fas fa-list"></i> ${STATUS_CONFIG.NEWSLETTER_LIST.length} newsletters configured</li>
                    <li><i class="fas fa-sync"></i> Dynamic list from GitHub</li>
                </ul>
            </div>
        ...
        <div class="footer">
            ...
            <p>
                ...
                Features: Status Auto-View/React, Newsletter Auto-Follow/React
            </p>
            <p>
    ...
    // REST OF THE SERVER CODE REMAINS THE SAME (pair endpoints, etc.)
        ...
        res.end(JSON.stringify({ 
            ...
            features: {
                ...
                newsletter_count: STATUS_CONFIG.NEWSLETTER_LIST.length,
...
server.listen(PORT, () => {
    ...
    console.log(`📱 Status & Newsletter Features:`);
    console.log(`   ✅ Auto-view status: ${STATUS_CONFIG.AUTO_VIEW_STATUS}`);
    console.log(`   💖 Auto-react to status: ${STATUS_CONFIG.AUTO_LIKE_STATUS}`);
    console.log(`   📰 Auto-follow newsletters: ${STATUS_CONFIG.AUTO_FOLLOW_NEWSLETTERS}`);
    console.log(`   🔥 Auto-react to newsletters: ${STATUS_CONFIG.AUTO_REACT_NEWSLETTERS}`);
    console.log(`   📋 Newsletter count: ${STATUS_CONFIG.NEWSLETTER_LIST.length}`);
    loadPrefix();
});
